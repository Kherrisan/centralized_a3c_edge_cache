...
class LFUAllocator:
    ...
    def choose_action(self, state, request):
        ...
        if np.nonzero(state[bs, :, file])[0].size != 0:
            ...
            self.access_state[bs, cache_index] += 1
            action = None
        else:
            # miss the cache
            if np.sum(state[bs, :, :]) >= self.bs_size:
                # replace the least frequently used cache entry with the request file
                lfu_index = np.argmin(self.access_state[bs, :])
                self.access_state[bs, lfu_index] = 1
                action = (bs, lfu_index, file)
            else:
                # enough cache size, found an empty cache entry
                cache_state = np.sum(state[bs, :, :], axis=1)
                empty_index = np.nonzero(1-cache_state)[0][0]
                self.access_state[bs, empty_index] = 1
                action = (bs, empty_index, file)
        return action


def test_lfu(env, h_parameter):
    episodes = h_parameter["e"]
    agent = LFUAllocator(env.state.shape, env.user_bs_mat)
    delay_per_ep = []
    hit_r_per_ep = []
